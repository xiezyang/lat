...
probe=$1
expected=$2
tmpdir=$(mktemp -d /tmp/wi1924-check-XXXXXX)
trap 'rm -rf "$tmpdir"' EXIT HUP INT TERM
[ -x "$probe" ] || exit 2
objdump -d -Mintel "$probe" >"$tmpdir/objdump.txt"
if grep -Eq '^[[:space:]]*[0-9a-f]+:[[:space:]]+62[[:space:]]' "$tmpdir/objdump.txt"; then
    echo "FAIL EVEX encoding" >&2
    exit 1
fi
mnemonics=$(objdump -d --no-show-raw-insn -Mintel "$probe" |
    awk '$2 ~ /^v[a-zA-Z0-9_]+$/ { print tolower($2) }' | sort -u)
allowed=$(printf '%s\n' vmovdqu vzeroall vzeroupper | sort -u)
if [ "$(printf '%s\n' "$mnemonics" | sort -u)" != "$allowed" ]; then
    echo "FAIL unexpected VEX mnemonics: $mnemonics" >&2
    exit 1
fi
test "$(grep -Ec '[[:space:]]vzeroall([[:space:]]|$)' "$tmpdir/objdump.txt")" -eq 1
test "$(grep -Ec '[[:space:]]vzeroupper([[:space:]]|$)' "$tmpdir/objdump.txt")" -eq 1
grep -Eq "[[:space:]]$expected([[:space:]]|$)" "$tmpdir/objdump.txt"
echo "PASS WI-1924 static VZEROALL/VZEROUPPER fixture"
