...
const uint64_t latx_vmovshdup_source_a[4] = {
    UINT64_C(0x7ff8000000000042), UINT64_C(0x7ff0000000000001),
    UINT64_C(0x0123456789abcdef), UINT64_C(0x8000000000000001),
};
const uint64_t latx_vmovshdup_source_b[4] = {
    UINT64_C(0x7ff0000000000042), UINT64_C(0x7ff8000000000001),
    UINT64_C(0xfedcba9876543210), UINT64_C(0x7ffffffffffffffe),
};
const uint64_t latx_vmovshdup_memory[4] = {
    UINT64_C(0x7ff8000000000042), UINT64_C(0x7ff0000000000001),
    UINT64_C(0x0123456789abcdef), UINT64_C(0x8000000000000001),
};

extern void latx_avx_single_vmovshdup_run(uint8_t *);
extern void latx_avx_single_vmovshdup_fault_xmm(uint8_t *);
extern void latx_avx_single_vmovshdup_fault_ymm(uint8_t *);
...
static int fault(const char *kind)
{
    ...
    if (kind[0] == 'x') {
        latx_avx_single_vmovshdup_fault_xmm(page + PAGE - 8);
    } else {
        latx_avx_single_vmovshdup_fault_ymm(page + PAGE - 16);
    }
    return 1;
}
...
int latx_avx_single_main(long argc, char **argv)
{
    ...
    latx_avx_single_vmovshdup_run(output);
