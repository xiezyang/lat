...
check_probe()
{
    ...
    "$script_dir/check-latx-avx-single-mnemonic.sh" "$1" vpunpcklqdq
}
...
marker_pc()
{
    nm -n "$1" | awk '$3 == "latx_avx_single_vpunpcklqdq_observe_marker" {
        sub(/^0+/, "", $1)
        print "0x" $1
    }'
}
...
generate)
    ...
    echo "PASS generated x86 vpunpcklqdq reference"
    ;;
verify)
    [ "$#" -eq 5 ] || usage
    ...
    if [ -z "$marker" ]; then
        echo "FAIL missing vpunpcklqdq observer marker" >&2
        exit 1
    fi
    env LATX_AVX_CPUID=0 LATX_AVX_TRACE=3 LATX_AVX_TRACE_YMM=15 \
        LATX_AVX_TRACE_YMM_INIT=1 \
    ...
    body=$(awk '
        /bool translate_vpunpcklxx\(/ { in_function = 1 }
        in_function { print }
        in_function && /^}/ { exit }
    ' "$source")
    qdq_branch=$(printf '%s\n' "$body" | awk '
        /if \(op == dt_X86_INS_VPUNPCKLQDQ\)/ { in_branch = 1 }
        in_branch { print }
        in_branch && /\/\* Original LASX path \*\// { exit }
    ')
    for required in 'if (1)' load_v128_from_ir1_mem_exact \
                    load_v256_from_ir1_mem_exact la_vilvl_d \
                    load_ymm_high128_shadow store_ymm_high128_shadow \
                    clear_ymm_high128_shadow; do
        if ! printf '%s\n' "$qdq_branch" | grep -Fq "$required"; then
            echo "FAIL vpunpcklqdq LSX branch misses $required" >&2
            exit 1
        fi
    done
    if printf '%s\n' "$qdq_branch" | grep -Eq '\bla_xv'; then
        echo "FAIL vpunpcklqdq LSX branch contains LASX" >&2
        exit 1
    fi
    dump=$(objdump -d "$latx" | awk '
        /<translate_vpunpcklxx>:/ { in_function = 1 }
        in_function { print }
        in_function && /^$/ { exit }
    ')
    if ! printf '%s\n' "$dump" | grep -Fq '<la_vilvl_d>'; then
        echo "FAIL compiled vpunpcklxx misses la_vilvl_d" >&2
        exit 1
    fi
    if printf '%s\n' "$dump" | grep -Fq '<la_xvilvl_d>'; then
        echo "FAIL compiled vpunpcklxx calls la_xvilvl_d" >&2
        exit 1
    fi
    echo "PASS vpunpcklqdq XMM/YMM register, memory, alias and fault differential"
    ;;
*) usage ;;
esac
