...
remote_host=${1:-xzy86}
remote_dir=${2:-/tmp/latx-avx-wi1896-vmovdqa}
local_output=${3:-/tmp/latx-avx-wi1896-vmovdqa.static}
script_dir=$(cd "$(dirname "$0")" && pwd)
stem=latx-avx-single-vmovdqa
ssh_args=()
scp_args=()
...
for file in \
    ...
    "$script_dir/check-latx-avx-wi1896-mnemonic.sh"; do
...
scp "${scp_args[@]}" \
    ...
    "$script_dir/$stem.c" \
    "$script_dir/latx-avx-single-common.h" \
    "$script_dir/check-latx-avx-wi1896-mnemonic.sh" \
...
docker run --rm --platform linux/amd64 \
    ...
    bash -ceu '
        gcc -std=c11 -O0 -Wall -Wextra -Werror \
            ...
            "/work/$STEM.S" \
            "/work/$STEM.c"
        chmod +x /work/check-latx-avx-wi1896-mnemonic.sh
        /work/check-latx-avx-wi1896-mnemonic.sh "/work/$STEM.static" vmovdqa
        "/work/$STEM.static" > "/work/$STEM.native"
        test "$(wc -c < "/work/$STEM.native")" -eq 384
        for case_name in \
            xmm-load-u xmm-store-u ymm-load-u ymm-store-u \
            xmm-load-cross xmm-store-cross ymm-load-cross ymm-store-cross \
            xmm-load-page xmm-store-page ymm-load-page ymm-store-page; do
            if "/work/$STEM.static" "$case_name" \
                >"/work/$STEM.$case_name.stdout" \
                2>"/work/$STEM.$case_name.stderr"; then
...
printf 'PASS xzy86 WI-1896 fixture: output=%s\n' "$local_output"
