...
const uint64_t wi1904_vmovhpd_source_a[2] = {
    UINT64_C(0x7ff8000000000042), UINT64_C(0x8000000000000001),
};
const uint64_t wi1904_vmovhpd_source_b[2] = {
    UINT64_C(0x7ff0000000000001), UINT64_C(0x0123456789abcdef),
};
const uint64_t wi1904_vmovhpd_seed[4] = {
...
extern void latx_avx_single_vmovhpd_run(uint8_t *, uint8_t *);
extern void latx_avx_single_vmovhpd_fault_load(uint8_t *);
extern void latx_avx_single_vmovhpd_fault_store(uint8_t *);
...
{
    if (argc == 1) {
        latx_avx_single_vmovhpd_run(output, memory);
    ...
    if (wi1904_streq(argv[1], "fault-load")) {
        latx_avx_single_vmovhpd_fault_load(crossing);
    } else {
        latx_avx_single_vmovhpd_fault_store(crossing);
    }
    return 5;
}
