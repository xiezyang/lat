...
extern void latx_avx_single_vrsqrtss_run(uint8_t *);
extern void latx_avx_single_vrsqrtss_fault_xmm(uint8_t *);
...
int latx_avx_single_main(long argc, char **argv)
{
    if (argc == 1 || (argc == 2 && streq(argv[1], "reference"))) {
        latx_avx_single_vrsqrtss_run(output);
    ...
    if (streq(argv[1], "fault-xmm")) {
        latx_avx_single_vrsqrtss_fault_xmm(page + 4096 - 4 + 1);
        return 90;
    }
    return 72;
}
