...
extern void latx_avx_single_vhsubps_run(uint8_t *);
extern void latx_avx_single_vhsubps_fault_xmm(uint8_t *);

extern void latx_avx_single_vhsubps_fault_ymm(uint8_t *);
...
int latx_avx_single_main(long argc, char **argv)
{
    if (argc == 1 || (argc == 2 && streq(argv[1], "reference"))) {
        latx_avx_single_vhsubps_run(output);
    ...
    if (streq(argv[1], "fault-xmm")) {
        latx_avx_single_vhsubps_fault_xmm(page + 4096 - 16 + 1);
        return 90;
    }
    if (streq(argv[1], "fault-ymm")) {
        latx_avx_single_vhsubps_fault_ymm(page + 4096 - 31);
        return 90;
    }
    return 72;
}
