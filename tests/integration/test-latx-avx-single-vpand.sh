...
check_probe()
{
    ...
    "$script_dir/check-latx-avx-single-mnemonic.sh" "$1" vpand
}
...
marker_pc()
{
    nm -n "$1" | awk '$3 == "latx_avx_single_vpand_observe_marker" {
        sub(/^0+/, "", $1)
        print "0x" $1
    }'
}
...
generate)
    ...
    echo "PASS generated x86 vpand reference"
    ;;
verify)
    [ "$#" -eq 5 ] || usage
    ...
    if [ -z "$marker" ]; then
        echo "FAIL missing vpand observer marker" >&2
        exit 1
    fi
    env LATX_AVX_CPUID=0 LATX_AVX_TRACE=3 LATX_AVX_TRACE_YMM=15 \
        LATX_AVX_TRACE_YMM_INIT=1 \
    ...
    body=$(awk '
        /bool translate_vpand\(/ { in_function = 1 }
        in_function { print }
        in_function && /^}/ { exit }
    ' "$source")
    printf '%s\n' "$body" | grep -Fq 'if (1)'
    ...
    if printf '%s\n' "$lsx_branch" | grep -Eq '\bla_xv'; then
        echo "FAIL vpand LSX branch contains LASX" >&2
        exit 1
    fi
    dump=$(objdump -d "$latx" | awk '
        /<translate_vpand>:/ { in_function = 1 }
        in_function { print }
        in_function && /^$/ { exit }
    ')
    if printf '%s\n' "$dump" | grep -Eq '<la_xv[^>]*>'; then
        echo "FAIL compiled vpand calls LASX generator" >&2
        exit 1
    fi
    echo "PASS vpand XMM/YMM register, memory, alias and fault differential"
    ;;
*) usage ;;
esac
