...
extern void latx_avx_single_vmovdqa_run(uint8_t *, uint8_t *);
extern void latx_avx_single_vmovdqa_fault_xmm_load(uint8_t *);
extern void latx_avx_single_vmovdqa_fault_xmm_store(uint8_t *);
extern void latx_avx_single_vmovdqa_fault_ymm_load(uint8_t *);
extern void latx_avx_single_vmovdqa_fault_ymm_store(uint8_t *);
...
static void handler(int sig, void *info, void *ctx)
{
    ...
    struct vmovdqa_fault_record record = {0};
...
{
    latx_avx_single_fill(p, PAGES * PAGE, UINT64_C(0x564d4f56445141));
...
int latx_avx_single_main(long argc, char **argv)
{
    ...
    prepare(page);
    if (argc == 1) {
        latx_avx_single_vmovdqa_run(output, page);
    ...
    if (same(argv[1], "xmm-load-u")) {
        address += 1;
        operation = latx_avx_single_vmovdqa_fault_xmm_load;
    } else if (same(argv[1], "xmm-store-u")) {
        address += 1;
        operation = latx_avx_single_vmovdqa_fault_xmm_store;
    } else if (same(argv[1], "ymm-load-u")) {
        address += 1;
        operation = latx_avx_single_vmovdqa_fault_ymm_load;
    } else if (same(argv[1], "ymm-store-u")) {
        address += 1;
        operation = latx_avx_single_vmovdqa_fault_ymm_store;
