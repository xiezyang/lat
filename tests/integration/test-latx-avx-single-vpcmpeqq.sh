...
check_probe()
{
    ...
    "$script_dir/check-latx-avx-single-mnemonic.sh" "$1" vpcmpeqq
}
...
marker_pc()
{
    nm -n "$1" | awk '$3 == "latx_avx_single_vpcmpeqq_observe_marker" {
        sub(/^0+/, "", $1)
        print "0x" $1
    }'
}
...
generate)
    ...
    if [ "$unique_states" -lt 8 ]; then
        echo "FAIL reference state diversity: $unique_states" >&2
        exit 1
    fi
    head -n 42 "$expected/state.txt" >"$expected/state-reg0.txt"
    tail -n 42 "$expected/state.txt" >"$expected/state-reg15.txt"
    ...
    echo "PASS generated x86 vpcmpeqq reference"
    ;;
verify)
    [ "$#" -eq 5 ] || usage
    ...
    if [ -z "$marker" ]; then
        echo "FAIL missing vpcmpeqq observer marker" >&2
        exit 1
    fi
    ...
    if [ "$(wc -l <"$tmpdir/state-reg0.txt")" -ne 42 ]; then
    ...
    if [ "$(wc -l <"$tmpdir/state-reg15.txt")" -ne 42 ]; then
    ...
    body=$(awk '
        /bool translate_vpcmpeqx\(/ { in_function = 1 }
        in_function { print }
        in_function && /^}/ { exit }
    ' "$source")
    printf '%s\n' "$body" | grep -Fq 'if (1)'
    ...
    if printf '%s\n' "$lsx_branch" | grep -Eq '\bla_xv'; then
        echo "FAIL vpcmpeqq LSX branch contains LASX" >&2
        exit 1
    fi
    dump=$(objdump -d "$latx" | awk '
        /<translate_vpcmpeqx>:/ { in_function = 1 }
        in_function && /<translate_vpcmpeqx>:/ { print; next }
        in_function && /^[[:xdigit:]]+ <[^>]+>:/ { exit }
        in_function { print }
    ')
    if printf '%s\n' "$dump" | grep -Fq '<la_xvseq_d>'; then
        echo "FAIL compiled vpcmpeqq calls LASX d-compare generator" >&2
        exit 1
    fi
    printf '%s\n' "$dump" | grep -Fq '<la_vseq_d>'
    echo "PASS vpcmpeqq XMM/YMM register, memory, alias and fault differential"
    ;;
*) usage ;;
esac
