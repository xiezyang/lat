...
remote_host=${1:-xzy86}
remote_dir=${2:-/tmp/latx-avx-wi1905}
local_output_dir=${3:-/tmp/wi1905-x86-fixtures}
...
for mnemonic in vmovhps vmovlps; do
    ...
    for file in \
        ...
        "$script_dir/check-latx-avx-wi1905-mnemonic.sh"; do
...
scp "${scp_args[@]}" \
    "$script_dir/latx-avx-single-runtime.S" \
    "$script_dir/latx-avx-single-vmovhps.S" \
    "$script_dir/latx-avx-single-vmovhps.c" \
    "$script_dir/latx-avx-single-vmovlps.S" \
    "$script_dir/latx-avx-single-vmovlps.c" \
    "$script_dir/latx-avx-single-common.h" \
    "$script_dir/check-latx-avx-wi1905-mnemonic.sh" \
...
docker run --rm --platform linux/amd64 -v "$remote_dir:/work" -w /work \
    latx-ci-baseline:latest bash -ceu '
        for mnemonic in vmovhps vmovlps; do
            ...
            chmod +x /work/check-latx-avx-wi1905-mnemonic.sh
            /work/check-latx-avx-wi1905-mnemonic.sh "/work/$stem.static" "$mnemonic"
            "/work/$stem.static" > "/work/$stem.native"
            test "$(wc -c < "/work/$stem.native")" -eq 128
            for case_name in load-cross store-cross; do
            ...
            for form in \
                ...
                "$mnemonic xmm0, xmm1, dword ptr [rax]" \
                "$mnemonic dword ptr [rax], xmm0"; do
                ...
                if gcc -c -mavx "$invalid" -o /tmp/$stem-invalid-memory.o \
                    ...
                    echo "FAIL $form accepted as a valid $mnemonic form" >&2
                    exit 1
                fi
            done
        done
...
printf 'PASS WI-1905 xzy86 fixture build: output_dir=%s\n' "$local_output_dir"
