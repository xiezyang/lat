...
extern void latx_avx_single_vpcmpistrm_run(uint8_t *);
extern void latx_avx_single_vpcmpistrm_fault(uint8_t *);
...
int latx_avx_single_main(long argc, char **argv)
{
    if (argc <= 1 || (argc == 2 && argv[1][0] == 'n')) {
        latx_avx_single_vpcmpistrm_run(output);
    ...
    if (argc == 2 && argv[1][0] == 'f') {
        ...
        latx_avx_single_vpcmpistrm_fault((uint8_t *)(mapping + PAGE - 8));
        return 90;
    }
    return 2;
}
