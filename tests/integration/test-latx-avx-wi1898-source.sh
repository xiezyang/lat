...
fixture_s=$root/tests/integration/latx-avx-single-vmovdqu.S
fixture_c=$root/tests/integration/latx-avx-single-vmovdqu.c
...
dqu=$(extract_function translate_vmovdqu_lsx)
[[ -n "$lsx" && -n "$dqu" ]]
for required in \
    ...
    'store_v256_to_ir1_mem_exact'; do
    printf '%s\n' "$lsx" | grep -Fq "$required" || {
        echo "FAIL VMOVDQU LSX audit missing: $required" >&2
        exit 1
    }
done
...
if printf '%s\n' "$lsx" | grep -Eq '\bla_xv'; then
    echo 'FAIL VMOVDQU LSX function contains LASX generator' >&2
    exit 1
fi
if printf '%s\n' "$lsx" | grep -Fq 'option_enable_lasx'; then
    echo 'FAIL VMOVDQU LSX function selects backend internally' >&2
    exit 1
fi

grep -Fq 'translate_register_lsx(dt_X86_INS_VMOVDQU, translate_vmovdqu_lsx)' "$dispatch"
grep -Fq 'TRANS_FUNC_GEN(VMOVDQU, vmovdqu)' "$dispatch"

for required in \
    'vmovdqu xmm0, xmmword ptr [rsi + DQU_SRC + 1]' \
    'vmovdqu ymm0, ymmword ptr [rsi + DQU_SRC + 1]' \
    'vmovdqu xmm0, xmm0' 'vmovdqu ymm0, ymm0' \
    'xmm-load-cross' 'ymm-store-cross' 'xmm-load-page' 'ymm-store-page'; do
...
printf '%s\n' "$dqu" | grep -Fq 'translate_vmovdqa_dqu_lsx(pir1, false)'
if printf '%s\n' "$dqu" | grep -Eq '\bla_xv'; then
    echo 'FAIL VMOVDQU LSX wrapper contains LASX generator' >&2
    exit 1
fi

echo 'PASS WI-1898 VMOVDQU LSX source and x86 fixture audit'
