...
if grep -Eq '^[[:space:]]*[0-9a-f]+:[[:space:]]+62[[:space:]]' \
    "$tmpdir/disassembly"; then
    echo "FAIL EVEX encoding is outside the supported VEX test range" >&2
    exit 1
fi

mnemonics=$(awk '$2 ~ /^v[a-zA-Z0-9_]+$/ { print $2 }' \
    "$tmpdir/disassembly" | sort -u)
expected_set=$(printf '%s\nvmovdqu\n' "$expected" | sort -u)
if [ "$mnemonics" != "$expected_set" ]; then
    echo "FAIL expected shift mnemonic '$expected' plus vmovdqu setup, found:" >&2
    printf '%s\n' "$mnemonics" >&2
    exit 1
fi

echo "PASS WI-1842 x86 fixture mnemonic: $expected"
