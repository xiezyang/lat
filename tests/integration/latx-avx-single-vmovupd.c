...
extern void latx_avx_single_vmovupd_run(uint8_t *, uint8_t *);
extern void latx_avx_single_vmovupd_fault_xmm_load(uint8_t *);
extern void latx_avx_single_vmovupd_fault_xmm_store(uint8_t *);
extern void latx_avx_single_vmovupd_fault_ymm_load(uint8_t *);
extern void latx_avx_single_vmovupd_fault_ymm_store(uint8_t *);
...
int latx_avx_single_main(long argc, char **argv)
{
    ...
    latx_avx_single_fill(page, PAGES * PAGE, UINT64_C(0x564d4f56555044));
    if (argc == 1) {
        latx_avx_single_vmovupd_run(output, page);
    ...
    if (same(argv[1], "xmm-load-u")) {
        address += 1; operation = latx_avx_single_vmovupd_fault_xmm_load;
    } else if (same(argv[1], "xmm-store-u")) {
        address += 1; operation = latx_avx_single_vmovupd_fault_xmm_store;
    } else if (same(argv[1], "ymm-load-u")) {
        address += 1; operation = latx_avx_single_vmovupd_fault_ymm_load;
    } else if (same(argv[1], "ymm-store-u")) {
        address += 1; operation = latx_avx_single_vmovupd_fault_ymm_store;
    } else if (same(argv[1], "xmm-load-cross") || same(argv[1], "xmm-store-cross")) {
        address += PAGE - 8;
        operation = same(argv[1], "xmm-load-cross")
                        ? latx_avx_single_vmovupd_fault_xmm_load
                        : latx_avx_single_vmovupd_fault_xmm_store;
    } else if (same(argv[1], "ymm-load-cross") || same(argv[1], "ymm-store-cross")) {
        address += PAGE - 16;
        operation = same(argv[1], "ymm-load-cross")
                        ? latx_avx_single_vmovupd_fault_ymm_load
                        : latx_avx_single_vmovupd_fault_ymm_store;
    } else if (same(argv[1], "xmm-load-page") || same(argv[1], "xmm-store-page") ||
               same(argv[1], "ymm-load-page") || same(argv[1], "ymm-store-page")) {
        address += PAGE;
