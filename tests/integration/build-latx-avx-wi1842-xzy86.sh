...
remote_host=xzy86
remote_dir=/tmp/latx-avx-wi1842-$mnemonic
local_output=/tmp/latx-avx-wi1842-$mnemonic.static
...
for file in \
    ...
    "$script_dir/check-latx-avx-wi1842-mnemonic.sh"; do
...
scp "${scp_args[@]}" \
    ...
    "$script_dir/$stem.c" \
    "$script_dir/latx-avx-single-common.h" \
    "$script_dir/check-latx-avx-wi1842-mnemonic.sh" \
...
docker run --rm --platform linux/amd64 \
    ...
    bash -ceu '
        gcc -std=c11 -O0 -Wall -Wextra -Werror \
            ...
            "/work/$STEM.S" \
            "/work/$STEM.c"
        chmod +x /work/check-latx-avx-wi1842-mnemonic.sh
        /work/check-latx-avx-wi1842-mnemonic.sh \
...
printf 'PASS xzy86 WI-1842 fixture: mnemonic=%s output=%s\n' \
