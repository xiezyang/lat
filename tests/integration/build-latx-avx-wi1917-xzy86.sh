...
files=("$script_dir/latx-avx-single-runtime.S" "$script_dir/latx-avx-single-common.h" "$script_dir/check-latx-avx-wi1917-mnemonic.sh")
for m in $mnemonics; do files+=("$script_dir/latx-avx-single-$m.S" "$script_dir/latx-avx-single-$m.c"); done
...
for m in $MNEMONICS; do
    ...
    chmod +x /work/check-latx-avx-wi1917-mnemonic.sh
    /work/check-latx-avx-wi1917-mnemonic.sh /work/$stem.static $m \
        > /work/$stem.static.stdout 2> /work/$stem.static.stderr
    set +e
    /work/$stem.static > /work/$stem.native 2> /work/$stem.native.stderr; n=$?
    /work/$stem.static fault > /work/$stem.fault.stdout 2> /work/$stem.fault.stderr; f=$?
    set -e
...
printf 'PASS WI-1917 xzy86 build/run output=%s\n' "$local_output_dir"
