...
extern uint8_t latx_avx_single_vcvtsd2ss_unmasked_site[];
extern void latx_avx_single_vcvtsd2ss_run(uint8_t *, uint8_t *);
extern void latx_avx_single_vcvtsd2ss_unmasked(uint8_t *, int);
...
{
    latx_avx_single_fill(page, PAGES * PAGE, UINT64_C(0x56435344325353));
    store64(page + 1, UINT64_C(0x4008000000000000));
    store64(page + 4090, UINT64_C(0x3ff0000000000000));
    store64(page + 8193, UINT64_C(0x0010000000000000));
    store64(page + 12287, UINT64_C(0x3fefffffffffffff));
    store64(page + 16385, UINT64_C(0x7ff8000000000000));
    store64(page + 20479, UINT64_C(0x3ff0000000000000));
}
...
static void signal_handler(int signal_number, void *siginfo, void *ucontext)
{
    ...
    record.signal_address_offset =
        (int64_t)(signal_address -
                  (uintptr_t)latx_avx_single_vcvtsd2ss_unmasked_site);
    record.rip_offset =
        (int64_t)(rip - (uintptr_t)latx_avx_single_vcvtsd2ss_unmasked_site);
...
int latx_avx_single_main(long argc, char **argv)
{
    ...
    if (equal(argv[1], "fpe-invalid")) {
        latx_avx_single_vcvtsd2ss_unmasked(0, 1);
    } else if (equal(argv[1], "fpe-precision")) {
        latx_avx_single_vcvtsd2ss_unmasked(0, 2);
    } else if (equal(argv[1], "fpe-overflow")) {
        latx_avx_single_vcvtsd2ss_unmasked(0, 3);
    } else {
        latx_avx_single_vcvtsd2ss_unmasked(0, 4);
    }
    return 90;
}
