...
remote_host=${1:-xzy86}
remote_dir=${2:-/tmp/latx-avx-wi1895-vmovapd}
local_output=${3:-/tmp/latx-avx-wi1895-vmovapd.static}
script_dir=$(cd "$(dirname "$0")" && pwd)
stem=latx-avx-single-vmovapd
ssh_args=()
scp_args=()
...
for file in \
    ...
    "$script_dir/check-latx-avx-wi1895-mnemonic.sh"; do
...
scp "${scp_args[@]}" \
    ...
    "$script_dir/$stem.c" \
    "$script_dir/latx-avx-single-common.h" \
    "$script_dir/check-latx-avx-wi1895-mnemonic.sh" \
...
docker run --rm --platform linux/amd64 \
    ...
    bash -ceu '
        gcc -std=c11 -O0 -Wall -Wextra -Werror \
            ...
            "/work/$STEM.S" \
            "/work/$STEM.c"
        chmod +x /work/check-latx-avx-wi1895-mnemonic.sh
        /work/check-latx-avx-wi1895-mnemonic.sh "/work/$STEM.static" vmovapd
...
printf 'PASS xzy86 WI-1895 fixture: output=%s\n' "$local_output"
