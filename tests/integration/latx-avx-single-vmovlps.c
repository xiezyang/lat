...
static long latx_vmovlps_syscall3(long number, long arg0, long arg1,
                                   long arg2)
{
    register long rax __asm__("rax") = number;
    register long rdi __asm__("rdi") = arg0;
...
static int latx_vmovlps_equal(const char *left, const char *right)
{
...
static int latx_vmovlps_fault(const char *kind)
{
    uint8_t *page = (uint8_t *)(uintptr_t)latx_vmovlps_syscall6(
    ...
    if ((uintptr_t)page >= (uintptr_t)-4095 ||
        latx_vmovlps_syscall3(LATX_SYS_MPROTECT, (long)(page + LATX_PAGE_SIZE),
                              LATX_PAGE_SIZE, LATX_PROT_NONE) < 0) {
        return 2;
    }
    uint8_t *cross_page = page + LATX_PAGE_SIZE - 4;
    if (latx_vmovlps_equal(kind, "load-cross")) {
        latx_avx_single_vmovlps_fault_load(cross_page);
    } else {
        latx_avx_single_vmovlps_fault_store(cross_page);
    }
    return 1;
}
...
{
    if (argc > 1) {
        return latx_vmovlps_fault(argv[1]);
    }
    latx_avx_single_vmovlps_run(output);
