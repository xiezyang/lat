...
output_dir=${1:-/tmp/wi1903-x86-fixtures}
remote_host=${2:-xzy86}
remote_dir=${3:-/tmp/latx-avx-wi1903}
script_dir=$root/tests/integration

bash "$script_dir/test-latx-avx-wi1903-source.sh"
bash -n \
    "$script_dir/check-latx-avx-wi1903-mnemonic.sh" \
    "$script_dir/build-latx-avx-wi1903-xzy86.sh" \
    "$script_dir/test-latx-avx-wi1903-source.sh" \
    "$script_dir/test-latx-avx-wi1903-fixtures.sh"
bash "$script_dir/build-latx-avx-wi1903-xzy86.sh" \
...
printf 'PASS WI-1903 xzy86 fixture set: count=2 manifest=%s\n' "$manifest"
