...
extern void latx_avx_single_vunpcklpd_run(uint8_t *, uint8_t *);
extern void latx_avx_single_vunpcklpd_fault_xmm(uint8_t *);
extern void latx_avx_single_vunpcklpd_fault_ymm(uint8_t *);
...
int latx_avx_single_main(long argc, char **argv)
{
    ...
    if (argc == 1) {
        latx_avx_single_vunpcklpd_run(output, 0);
    ...
    if (same(argv[1], "xmm-cross-8")) {
        ...
        latx_avx_single_vunpcklpd_fault_xmm(fault_target);
    } else {
        fault_name = "ymm-cross-16";
        fault_target = page + PAGE - 16;
        latx_avx_single_vunpcklpd_fault_ymm(fault_target);
    }
    return 5;
}
