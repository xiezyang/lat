...
fault_cases='xmm-cross-1 xmm-cross-7 ymm-cross-1 ymm-cross-7'
...
check_probe()
{
    ...
    "$script_dir/check-latx-avx-single-mnemonic.sh" "$1" vpbroadcastq
}
...
marker_pc()
{
    nm -n "$1" | awk '$3 == "latx_avx_single_vpbroadcastq_observe_marker" {
        sub(/^0+/, "", $1)
        print "0x" $1
    }'
}
...
check_sources()
{
    source=$1
    body=$(awk '
        /bool translate_vpbroadcastq\(/ { in_function = 1 }
        in_function { print }
        in_function && /^}/ { exit }
    ' "$source")
    printf '%s\n' "$body" | grep -Fq 'if (1)'
    ...
    if printf '%s\n' "$lsx_branch" | grep -Eq '\bla_xv'; then
        echo "FAIL vpbroadcastq LSX branch contains LASX" >&2
        exit 1
    fi
}
...
check_host_code()
{
    ...
    dump=$(objdump -d "$latx" | awk '
        /<translate_vpbroadcastq>:/ { in_function = 1 }
        in_function && /<translate_vpbroadcastq>:/ { print; next }
        in_function && /^$/ { exit }
        in_function && /^[[:space:]]*[[:xdigit:]]+ <[^>]+>:/ { exit }
        in_function { print }
    ')
    if printf '%s\n' "$dump" | grep -Eq '<la_xv[^>]*>'; then
        echo "FAIL compiled vpbroadcastq calls LASX generator" >&2
        exit 1
    fi
...
generate)
    ...
    echo "PASS generated x86 vpbroadcastq reference"
    ;;
verify)
    [ "$#" -eq 5 ] || usage
    ...
    if [ -z "$marker" ]; then
        echo "FAIL missing vpbroadcastq observer marker" >&2
        exit 1
    fi
    ...
    echo "PASS vpbroadcastq XMM/YMM register, m64, alias and fault differential"
    ;;
*) usage ;;
esac
