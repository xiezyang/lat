...
[ "$target_count" -eq 4 ] || {
    echo "FAIL expected two normal and two fault $expected instructions, found $target_count" >&2
    exit 1
}
grep -Eq "[[:space:]]$expected xmm[^,]*,[[:space:]]*XMMWORD PTR" "$tmpdir/disassembly"
grep -Eq "[[:space:]]$expected ymm[^,]*,[[:space:]]*YMMWORD PTR" "$tmpdir/disassembly"
echo "PASS WI-1907 x86 fixture mnemonic: $expected"
