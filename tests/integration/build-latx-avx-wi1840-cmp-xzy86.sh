...
remote_host=xzy86
remote_dir=/tmp/latx-avx-wi1840-$mnemonic
local_output=/tmp/latx-avx-wi1840-$mnemonic.static
...
case "$mnemonic" in
    vpcmpeqb|vpcmpeqw|vpcmpeqd|vpcmpgtb|vpcmpgtw|vpcmpgtd|vpcmpgtq) ;;
    *) echo "FAIL unsupported WI-1840 fixture: $mnemonic" >&2; exit 2 ;;
esac
...
for file in \
    ...
    "$script_dir/$stem.S" \
    "$script_dir/$stem.c" \
    "$script_dir/latx-avx-single-vintcmp-template.S" \
    "$script_dir/latx-avx-single-vintcmp-template.c" \
    "$script_dir/latx-avx-single-common.h" \
    "$script_dir/check-latx-avx-single-mnemonic.sh"; do
    if [[ ! -f "$file" ]]; then
        echo "FAIL missing WI-1840 fixture: $file" >&2
        exit 2
    fi
done
...
printf 'PASS WI-1840 native fixture build: mnemonic=%s output=%s\n' \
