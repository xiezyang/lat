...
expected=$4
output=$5
source_file=$(cd "$(dirname "$0")/../.." && pwd)/target/i386/latx/translator/tr-avx-cvt.c
mkdir -p "$output"
...
lsx_body=$(extract_function translate_vcvtsd2ss_lsx)
lasx_body=$(extract_function translate_vcvtsd2ss)
printf '%s\n' "$lsx_body" | grep -Fq 'la_fcvt_s_d'
printf '%s\n' "$lsx_body" | grep -Fq 'clear_ymm_high128_shadow'
if printf '%s\n' "$lsx_body" | grep -Eq '\bla_xv'; then
    echo 'FAIL VCVTSD2SS LSX source contains LASX generator' >&2
    exit 1
fi
printf '%s\n' "$lasx_body" | grep -Fq 'la_fcvt_s_d'
...
run_lasx reference
run_lsx reference
cmp "$expected/reference.bin" "$output/lasx-reference.bin"
cmp "$expected/reference.bin" "$output/lsx-reference.bin"
for name in fpe-invalid fpe-precision; do
    run_lasx "$name"
    run_lsx "$name"
    cmp "$expected/$name.bin" "$output/lasx-$name.bin"
...
sha256sum "$latx" "$probe" "$source_file" \
    ...
    "$expected/fpe-precision.bin" "$output/lasx-reference.bin" \
    "$output/lsx-reference.bin" "$output/lasx-fpe-invalid.bin" \
    "$output/lsx-fpe-invalid.bin" "$output/lasx-fpe-precision.bin" \
    "$output/lsx-fpe-precision.bin" >"$output/hashes.sha256"
echo 'PASS VCVTSD2SS x86/LASX/forced-LSX three-way regression'
