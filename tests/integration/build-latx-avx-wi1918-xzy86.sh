...
remote_host=${1:-xzy86}
remote_dir=${2:-/tmp/latx-avx-wi1918-x86}
local_output=${3:-$(mktemp -d /tmp/wi1918-x86-XXXXXX)}
...
fixture_dir=${WI1918_FIXTURE_DIR:-$script_dir/wi1918-fixtures}
...
scp "${scp_args[@]}" "$script_dir/latx-avx-single-runtime.S" \
    ...
    "$script_dir/check-latx-avx-wi1918-mnemonic.sh" \
    "$fixture_dir"/latx-avx-single-* \
...
for m in $MNEMONICS; do
    ...
    test "$build_rc" -eq 0
    chmod +x /work/check-latx-avx-wi1918-mnemonic.sh
    set +e
    /work/check-latx-avx-wi1918-mnemonic.sh "$binary" "$m" > /work/$stem.static.stdout 2> /work/$stem.static.stderr
    static_rc=$?
    set -e
    printf "%s\n" "$static_rc" > /work/$stem.static.exit
    test "$static_rc" -eq 0
    objdump -d -Mintel "$binary" > /work/$stem.objdump
    for mode in normal load-xmm-zero load-ymm-zero store-xmm-zero store-ymm-zero load-xmm-one load-ymm-mix store-xmm-one store-ymm-mix; do
        ...
        case "$mode" in
            normal|*-zero) test "$rc" -eq 0 ;;
            *) test "$rc" -eq 139 ;;
        esac
        if [ "$mode" = normal ]; then
            test "$(wc -c < /work/$stem.$mode.stdout)" -eq 768
        else
            test "$(wc -c < /work/$stem.$mode.stdout)" -eq 16
        fi
    done
...
printf 'PASS WI-1918 xzy86 baseline output=%s\n' "$local_output"
