...
[ "$target_count" -eq 8 ] || {
    echo "FAIL expected six normal and two fault $expected instructions, found $target_count" >&2
    exit 1
}
grep -Eq "[[:space:]]$expected xmm[^,]*,[[:space:]]*xmm" "$tmpdir/disassembly"
grep -Eq "[[:space:]]$expected ymm[^,]*,[[:space:]]*ymm" "$tmpdir/disassembly"
grep -Eq "[[:space:]]$expected xmm[^,]*,[[:space:]]*(XMMWORD PTR|xmm)" "$tmpdir/disassembly"
echo "PASS WI-1909 x86 fixture mnemonic: $expected"
