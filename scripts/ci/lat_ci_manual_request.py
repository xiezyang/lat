...
"""Resolve a manually approved LAT pull request in the personal fork."""
...
def get_json(path, token, opener=urllib.request.urlopen):
    """Read GitHub API JSON with the workflow token rather than anonymously."""
...
def resolve_pull_request(pr_number_value, target, fetch_json):
    ...
    if source_repository != PERSONAL_REPOSITORY:
        raise LatCIRequestError(f"The PR head must be {PERSONAL_REPOSITORY}")
...
def write_github_output(values, output_path):
    """Write validated single-line values to GitHub Actions job outputs."""
