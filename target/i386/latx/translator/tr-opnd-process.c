                                                 ...
                                                 IR2_OPND mem_opnd, int mem_imm)
{
    ...
    latx_load_f64(ir2_fraction, mem_opnd, mem_imm);
...
void latx_load_f64(IR2_OPND dest, IR2_OPND base, int disp)
{
    ...
    if (si12_overflow(disp)) {
        li_d(value, disp);
        la_add_d(address, base, value);
    } else {
        la_addi_d(address, base, disp);
    }
    la_andi(value, address, 7);
    la_bne(value, zero_ir2_opnd, bytes);
    la_ld_d(value, address, 0);
    la_b(done);
    la_label(bytes);
    /* Read only the requested bytes, including at guest page boundaries. */
    latx_load_u64_bytes(value, byte, address, 0);
    la_label(done);
    la_movgr2fr_d(dest, value);
...
IR2_OPND load_u64_from_ir1_mem_exact(IR1_OPND *opnd)
{
    ...
    load_ireg_from_ir1_mem(value, opnd, ZERO_EXTENSION, false);
    return value;
}
...
void store_u64_to_ir1_mem_exact(IR2_OPND value, IR1_OPND *opnd)
{
    ...
    lsassert(ir1_opnd_is_mem(opnd) && ir1_opnd_size(opnd) == 64);
    store_ireg_to_ir1_mem(value, opnd, false);
}
...
void store_v128_to_ir1_mem_exact(IR2_OPND value, IR1_OPND *opnd)
{
    ...
    latx_store_v128(value, address, 0);
    ra_free_temp(address);
}
