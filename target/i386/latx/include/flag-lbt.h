/*
 * SPDX-FileCopyrightText: 2021-2026 LAT Project Authors
 *
 * SPDX-License-Identifier: GPL-2.0-only
 */

#ifndef _FLAG_LBT_H_
#define _FLAG_LBT_H_

#include "ir1.h"
#include "ir2.h"
#include <stdint.h>

bool generate_eflag_by_lbt(IR2_OPND dest, IR2_OPND src0, IR2_OPND src1,
                           IR1_INST *pir1, bool is_imm);

void get_eflag_condition(IR2_OPND *value, IR1_INST *pir1);
void latx_write_eflags(IR2_OPND value, uint8_t mask);
void latx_read_eflags(IR2_OPND value, uint8_t mask);
void latx_set_eflag_condition(IR2_OPND dest, int condition);
void latx_write_top(IR2_OPND value);
void latx_write_top_const(int value);
void latx_read_top(IR2_OPND value);
void latx_inc_top(void);
void latx_dec_top(void);

#define GENERATE_EFLAG_IR2_1(ir2_opcode) do{ \
    if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 8) { \
        ir2_opcode##_b(src0); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 16) { \
        ir2_opcode##_h(src0); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 32) { \
        ir2_opcode##_w(src0); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 64) { \
        ir2_opcode##_d(src0); \
    } \
} while(0)

#define GENERATE_EFLAG_IR2_2(ir2_opcode) do{ \
    IR2_OPND temporary_src1 = src1; \
    if (ir2_opnd_is_imm(&src1)) { \
        temporary_src1 = ra_alloc_itemp(); \
        load_ireg_from_ir1_2(temporary_src1, ir1_get_opnd(pir1, 0) + 1, SIGN_EXTENSION, false); \
    } \
    if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 8) { \
        ir2_opcode##_b(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 16) { \
        ir2_opcode##_h(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 32) { \
        ir2_opcode##_w(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 64) { \
        ir2_opcode##_d(src0, temporary_src1); \
    } \
    if (ir2_opnd_is_imm(&src1)) { \
        ra_free_temp(temporary_src1); \
    } \
} while(0)

#define GENERATE_EFLAG_IR2_2_U(ir2_opcode) do{ \
    IR2_OPND temporary_src1 = src1; \
    if (ir2_opnd_is_imm(&src1)) { \
        temporary_src1 = ra_alloc_itemp(); \
        load_ireg_from_ir1_2(temporary_src1, ir1_get_opnd(pir1, 0) + 1, SIGN_EXTENSION, false); \
    } \
    if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 8) { \
        ir2_opcode##_bu(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 16) { \
        ir2_opcode##_hu(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 32) { \
        ir2_opcode##_wu(src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 64) { \
        ir2_opcode##_du(src0, temporary_src1); \
    } \
    if (ir2_opnd_is_imm(&src1)) { \
        ra_free_temp(temporary_src1); \
    } \
} while(0)

#define GENERATE_EFLAG_IR2_2_R(ir2_opcode) do{ \
    IR2_OPND temporary_src1 = src1; \
    if (ir2_opnd_is_imm(&src1)) { \
        temporary_src1 = ra_alloc_itemp(); \
        load_ireg_from_ir1_2(temporary_src1, ir1_get_opnd(pir1, 0) + 1, SIGN_EXTENSION, false); \
    } \
    if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 8) { \
        la_append_ir2_opnd2(ir2_opcode, src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 16) { \
        la_append_ir2_opnd2(ir2_opcode + 1, src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 32) { \
        la_append_ir2_opnd2(ir2_opcode + 2, src0, temporary_src1); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 64) { \
        la_append_ir2_opnd2(ir2_opcode + 3, src0, temporary_src1); \
    } \
    if (ir2_opnd_is_imm(&src1)) { \
        ra_free_temp(temporary_src1); \
    } \
} while(0)

#define GENERATE_EFLAG_IR2_2_I(ir2_opcode) do { \
    lsassert(ir2_opnd_is_imm(&src1)); \
    uint32 imm = ir2_opnd_imm(&src1); \
    if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 8) { \
        ir2_opcode##_b(src0, imm); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 16) { \
        ir2_opcode##_h(src0, imm); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 32) { \
        ir2_opcode##_w(src0, imm); \
    } \
    else if (ir1_opnd_size(ir1_get_opnd(pir1, 0)) == 64) { \
        ir2_opcode##_d(src0, imm); \
    } \
} while (0)

#endif
